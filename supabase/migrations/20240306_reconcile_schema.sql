...
-- Rename user_id to customer_id in bookings
...
-- Remove payment_status and payment_id from bookings (to be handled in a separate payments table later)
...
-- Remove business-related columns from profiles
...
-- Rename phone to phone_number in profiles
ALTER TABLE public.profiles RENAME COLUMN phone TO phone_number;

-- Rename owner_profile_id to owner_id in businesses
ALTER TABLE public.businesses RENAME COLUMN owner_profile_id TO owner_id;

-- Rename business_category to category_id in businesses
ALTER TABLE public.businesses RENAME COLUMN business_category TO category_id;

-- Rename contact_number to phone in businesses
ALTER TABLE public.businesses RENAME COLUMN contact_number TO phone;

-- Rename contact_email to email in businesses
ALTER TABLE public.businesses RENAME COLUMN contact_email TO email;

-- Rename postal_code to zip_code in businesses
ALTER TABLE public.businesses RENAME COLUMN postal_code TO zip_code;

-- Rename cover_image_url to banner_url in businesses
ALTER TABLE public.businesses RENAME COLUMN cover_image_url TO banner_url;

-- Remove duplicate logo_url column from businesses
...
-- Add services column to staff (if it doesn't exist - your description is ambiguous)
ALTER TABLE public.staff ADD COLUMN IF NOT EXISTS services text[] NULL;

-- Update user_role enum (add 'staff')
ALTER TYPE public.user_role ADD VALUE IF NOT EXISTS 'staff';

-- Update booking_status enum (add 'no-show')
ALTER TYPE public.booking_status ADD VALUE IF NOT EXISTS 'no-show';

-- Update service_status enum
...
ALTER TYPE public.service_status RENAME VALUE 'deleted' TO 'archived'; -- Assuming you want to keep 'archived'
